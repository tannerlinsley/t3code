...
const toSqlError = (cause: unknown, message: string) => new SqlError({ cause, message });
    ...
    const makeConnection = Effect.gen(function* () {
      ...
      const prepareCache = yield* Cache.make({
        ...
        lookup: (sql: string) =>
          Effect.try({
            try: () => db.prepare(sql),
            catch: (cause) => toSqlError(cause, "Failed to prepare statement"),
          }),
      });
      ...
      const runStatement = (
        ...
        Effect.withFiber<ReadonlyArray<any>, SqlError>((fiber) => {
          ...
          try {
            ...
            return Effect.fail(toSqlError(cause, "Failed to execute statement"));
          }
        });
        ...
        Effect.acquireUseRelease(
          ...
          (statement) =>
            Effect.try({
              ...
              catch: (cause) => toSqlError(cause, "Failed to execute statement"),
            }),
          (statement) =>
            Effect.sync(() => {
              if (hasRows(statement)) {
